...
{
    public partial class KITStockINFormAdd : Form
    {
        SQLConnect cnn = new SQLConnect();
        SQLConnectOBS cnnOBS = new SQLConnectOBS();
        KITStockINForm fgrid;

        string ErrorText;

        public KITStockINFormAdd(KITStockINForm fg)
        {
            InitializeComponent();
            this.cnn.Connection();
            this.cnnOBS.Connection();
            this.fgrid = fg;
            this.Shown += KITStockINFormAdd_Shown;
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            dtSQLConds.Rows.Add("SurName IS NOT ", "NULL");
            ...
            try
            {
                ...
                string SQLQuery = "SELECT T1.ItemCode, ItemName, T1.Remark, SUM(ReceiveQty) AS ReceiveQty FROM " +
                    "\n(SELECT ItemCode, ReceiveQty, Remark, CreateDate, CreateUser FROM prgalltransaction WHERE TypeCode=1 AND LocCode='MC1' AND GRICode=50) T1 " +
                    "\nINNER JOIN (SELECT * FROM mstitem WHERE DelFlag=0 AND ItemType=2) T2 " +
                    "\nON T1.ItemCode=T2.ItemCode " +
                    "\nLEFT JOIN (SELECT * FROM mststaff WHERE DelFlag=0 AND UserGroupID IN (7, 9)) T3 " +
                    "\nON T1.CreateUser=T3.StaffId \n" + SQLConds +
        ...
        private void KITStockINFormAdd_Shown(object sender, EventArgs e)
        {
        ...
        private void TakingAlreadyRecQty()
        {
            ...
            try
            {
                ...
                string SQLQuery = "SELECT Code, Remarks, SUM(ReceiveQty) AS ReceiveQty from tbSDMCAllTransaction " +
                    "\nWHERE CancelStatus = 0 AND Funct =1 AND LocCode='KIT3' " +
