        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string SQLQuery = "DECLARE @SDate AS date = '2023-11-01' " +
                    ...
                    "\nON T1.Code=T4.Code " +
                    "\nLEFT JOIN " +
                    "\n(\tSELECT mstitem.ItemCode, ItemName, MatTypeName, UnitPrice FROM ["+cnnOBS.server+"].["+cnnOBS.db+"].dbo.mstitem " +
                    "\n\tLEFT JOIN ["+cnnOBS.server+"].["+cnnOBS.db+"].dbo.MstMatType ON mstitem.MatTypeCode=MstMatType.MatTypeCode " +
                    "\n\tLEFT JOIN (SELECT mstpurchaseprice.ItemCode, UnitPrice FROM ["+cnnOBS.server+"].["+cnnOBS.db+"].dbo.mstpurchaseprice " +
                    "\n\tINNER JOIN (SELECT ItemCode, MAX(EffDate) AS MaxEffDate FROM ["+cnnOBS.server+"].["+cnnOBS.db+"].dbo.mstpurchaseprice WHERE DelFlag = 0 GROUP BY ItemCode) T1 " +
