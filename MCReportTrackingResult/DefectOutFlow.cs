...
{
    public partial class DefectOutFlow : Form
    {
        SQLConnect cnn = new SQLConnect();
        public DefectOutFlow()
        {
            InitializeComponent();
            cnn.Connection();
            this.Load += DefectOutFlow_Load;
            btnserch.Click += Btnserch_Click;
            dgvDefectOutFlow.CellDoubleClick += DgvDefectOutFlow_CellDoubleClick;
        ...
        {
            var cell1 = dgvDefectOutFlow.CurrentCell;
            if (cell1 == null) return;
            int rowIndex = cell1.RowIndex;
            int colIndex = cell1.ColumnIndex;
            if (rowIndex < 0 || colIndex < 0) return;
            string colName = dgvDefectOutFlow.Columns[colIndex].Name;
            ...
            var cell = dgvDefectOutFlow.Rows[rowIndex].Cells[colName];
            ...
            string type = dgvDefectOutFlow.Rows[rowIndex].Cells["colQC"].Value?.ToString() ?? "";
            if (type == "" || type == "Gap")
            {
            ...
            string category = "";
            if (rowIndex <= 2)
                category = dgvDefectOutFlow.Rows[0].Cells["colCategory"].Value?.ToString() ?? "";
            else if (rowIndex >= 3)
            ...
            if (MessageBox.Show("Are you sure you want to delete this record?", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                {
                    cnn.con.Open();
                    SqlCommand cmd = new SqlCommand("Delete from tbDefectOutFlow Where Category = @Category And Type = @Type And MonthYear = @MonthYear", cnn.con);
        ...
        {
            var cell = dgvDefectOutFlow.CurrentCell;
            if (cell == null)
            {
        ...
        private void OpendUpdateFrom(int rowIndex, int colIndex)
        {
            ...
            if (!DateTime.TryParseExact(mmyyyy, "MMyyyy",
                System.Globalization.CultureInfo.InvariantCulture,
                System.Globalization.DateTimeStyles.None, out DateTime monthYear)) return;
            using (var form = new EditDefectOutFlow(category, type, monthYear))
            {
        ...
        private void DgvDefectOutFlow_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void OpenInsertForm(int rowIndex, int colIndex)
        {
            ...
            using (var form = new InsertDefectOutFlow(category, type, monthYear))
            {
                if (form.ShowDialog(this) == DialogResult.OK)
                    Btnserch_Click(null, null);
            }
        ...
        private void Btnserch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlDataAdapter adapter = new SqlDataAdapter(" Select * From tbDefectOutFlow Where Status = 1 " + queryCondition, cnn.con);
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (dgvDefectOutFlow.Columns.Contains(colName))
                        {
                            for (int j = 0; dgvDefectOutFlow.Rows.Count > j; j++)
                            {

                                if (j < 3 && dt.Rows[i]["Category"].ToString() == dgvDefectOutFlow.Rows[0].Cells["colCategory"].Value.ToString() && dt.Rows[i]["Type"].ToString() == dgvDefectOutFlow.Rows[j].Cells["colQC"].Value.ToString())
                                {
                                    dgvDefectOutFlow.Rows[j].Cells[colName].Value = dt.Rows[i]["Qty"];
                                }
                                if (j >= 3 && dt.Rows[i]["Category"].ToString() == dgvDefectOutFlow.Rows[3].Cells["colCategory"].Value.ToString() && dt.Rows[i]["Type"].ToString() == dgvDefectOutFlow.Rows[j].Cells["colQC"].Value.ToString())
                                {
                                    dgvDefectOutFlow.Rows[j].Cells[colName].Value = dt.Rows[i]["Qty"];
                                }
        ...
        private void PlotAssyChartData()
        {
            var chart = chartAssy;
            chart.Series.Clear();

            // PLAN as Bar (Column)
            var planSeries = new Series("flow out to Assy PLAN")
            {
            ...
            var actualSeries = new Series("flow out to Assy ACTUAL")
            {
     ...
     .ToList();
            ...
            for (int i = 0; i < monthCols.Count; i++)
            {
                ...
                var actualVal = dgvDefectOutFlow.Rows[1].Cells[monthCols[i].Name].Value;
        ...
        private void PlotS3NChartData()
        {
            var chart = chartS3n;
            chart.Series.Clear();

            // PLAN as Bar (Column)
            var planSeries = new Series("flow out to S3N PLAN")
            {
                ChartType = SeriesChartType.Column,
                Color = Color.SkyBlue,
                BorderWidth = 2
            ...
            // ACTUAL as Line with markers
            var actualSeries = new Series("flow out to S3N ACTUAL")
            {
                ChartType = SeriesChartType.Line,
            ...
            for (int i = 0; i < monthCols.Count; i++)
            {
                ...
                var planVal = dgvDefectOutFlow.Rows[3].Cells[monthCols[i].Name].Value;
                if (planVal != null && double.TryParse(planVal.ToString(), out double p))
                    plan[i] = p;
                else
                    plan[i] = null;
                ...
                var actualVal = dgvDefectOutFlow.Rows[4].Cells[monthCols[i].Name].Value;
                if (actualVal != null && double.TryParse(actualVal.ToString(), out double a))
                    actual[i] = a;
                else
                    actual[i] = null;
        ...
        private void CalculateTotals()
        {
            ...
            foreach (DataGridViewRow row in dgvDefectOutFlow.Rows)
            {
                double sum = 0;
        ...
        {
            foreach (DataGridViewColumn col in dgvDefectOutFlow.Columns)
            {
                if (!col.Name.StartsWith("col_") || col.Name == "colTotal") continue;

                // Crimping PPM: rows 0=Plan, 1=Actual, 2=Gap
                object planVal = dgvDefectOutFlow.Rows[0].Cells[col.Name].Value;
                object actualVal = dgvDefectOutFlow.Rows[1].Cells[col.Name].Value;
                    ...
                    actualVal != null && actualVal.ToString() != "")
                {
                    if (decimal.TryParse(planVal.ToString(), out decimal p1) &&
                        ...
                        dgvDefectOutFlow.Rows[2].Cells[col.Name].Value = p1 - a1;
                }

                // Pressing PPM: rows 3=Plan, 4=Actual, 5=Gap
                object planVal2 = dgvDefectOutFlow.Rows[3].Cells[col.Name].Value;
                object actualVal2 = dgvDefectOutFlow.Rows[4].Cells[col.Name].Value;
                    ...
                    actualVal2 != null && actualVal2.ToString() != "")
                {
                    if (decimal.TryParse(planVal2.ToString(), out decimal p2) &&
                        ...
                        dgvDefectOutFlow.Rows[5].Cells[col.Name].Value = p2 - a2;
                }

                dgvDefectOutFlow.Rows[0].Cells["colQC"].Style.BackColor = Color.LightGreen;
                dgvDefectOutFlow.Rows[2].Cells["colQC"].Style.BackColor = Color.Red;
                dgvDefectOutFlow.Rows[3].Cells["colQC"].Style.BackColor = Color.LightGreen;
                ...
                for (int i = 3; i < dgvDefectOutFlow.ColumnCount - 1; i++)
                {
                    ...
                    var typeCell2 = dgvDefectOutFlow.Rows[4].Cells[i];
                    if (typeCell.Value != null && double.TryParse(typeCell.Value.ToString(), out double d))
                    {
                        if (d < 0)
                        {
                            dgvDefectOutFlow.Rows[2].Cells[i].Style.BackColor = Color.Red;
                        }
                    }
                    if (typeCell2.Value != null && double.TryParse(typeCell2.Value.ToString(), out double d2))
                    {
                        if (d2 < 0)
                        {
                            dgvDefectOutFlow.Rows[4].Cells[i].Style.BackColor = Color.Red;
                        }
                    }
                }
            }
        ...
        private void BuildGrid(DateTime start, DateTime end)
        {
            ...
            for (int i = dgvDefectOutFlow.Columns.Count - 1; i >= 0; i--)
            {
                string name = dgvDefectOutFlow.Columns[i].Name;
                if (name != "colNo" && name != "colCategory" && name != "colQC")
                    dgvDefectOutFlow.Columns.RemoveAt(i);
            }
            ...
            while (current <= endMonth)
            {
                ...
                dgvDefectOutFlow.Columns.Add(col);
                current = current.AddMonths(1);
            }

            // Add TOTAL column
            dgvDefectOutFlow.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "colTotal",
                HeaderText = "TOTAL",
                Width = 80,
            ...
            dgvDefectOutFlow.Rows[0].Cells["colCategory"].Value = "Flow out to ASSY";
            dgvDefectOutFlow.Rows[0].Cells["colQC"].Value = "Plan";
            dgvDefectOutFlow.Rows[1].Cells["colQC"].Value = "Actual";
            dgvDefectOutFlow.Rows[2].Cells["colQC"].Value = "Gap";

            dgvDefectOutFlow.Rows[3].Cells["colNo"].Value = "2";
            dgvDefectOutFlow.Rows[3].Cells["colCategory"].Value = "Flow out to S3N";
