    ...
    public partial class SemiPressInputByManualForm : Form
    {
        ...
        private void TxtWipNameS_Leave(object sender, EventArgs e)
        {
            if (txtWipNameS.Text.Trim() != "")
            {
                ...
                {
                    //Get all Semi
                    SqlDataAdapter da = new SqlDataAdapter("SELECT ItemCode, ItemName, Remarks1, Remarks2, Remarks3 FROM tbMasterItem WHERE LEN(ItemCode)>4 AND ItemName LIKE '%" + txtWipNameS.Text + "%';", cnn.con);
