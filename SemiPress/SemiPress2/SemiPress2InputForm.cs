...
{
    public partial class SemiPress2InputForm : Form
    {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            if (PosNo.Trim() != "" && WipCode.ToString().Trim() != "" && Qty.ToString().Trim() != "" && BoxNo.ToString().Trim() != "")
                            {
                                ...
                                cnn.con.Open();
                                SqlDataAdapter sda = new SqlDataAdapter("SELECT COUNT(*) FROM tbSemiPress2 WHERE PosNo ='" + PosNo + "' AND BoxNo='" + BoxNo + "' AND DeleteState='0';", cnn.con);
                                ...
                                {
                                    SqlDataAdapter sda1 = new SqlDataAdapter("SELECT PosCNo, WIPCode, PosCQty, PosPDelDate FROM tbPOSDetailofMC WHERE PosCNo ='" + PosNo + "' AND WipCode='" + WipCode + "';", cnn.con);
                                    PosDetails = new DataTable();
                                    sda1.Fill(PosDetails);
                                    if (PosDetails.Rows.Count > 0)
                                    {
                                        txtBarcode.Focus();
                                        SemiPress2InputConfirmForm Sp2icf = new SemiPress2InputConfirmForm(this);
                                        Sp2icf.ShowDialog();
        ...
        private void SemiPress2InputForm_Load(object sender, EventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (dgvScannedTag.Rows.Count > 0)
            {
                ...
                {
                    SqlCommand myCommand = new SqlCommand("SELECT *FROM tbSemiPress2 WHERE RegDate=(SELECT max(RegDate) FROM tbSemiPress2);", cnn.con);
                    cnn.con.Open();
                    ...
                    for (int i = 0; i < dgvScannedTag.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            try
                            {
                                cmd = new SqlCommand("INSERT INTO tbSemiPress2 (PosNo, WipCode, WipDes, BoxNo, Qty, Remarks, RegDate, RegBy, UpdateDate, UpdateBy, SysNo, DeleteState) VALUES (@Pn, @wc, @wd, @bn,@qty, @rm, @Rd, @Rb, @Ud, @Ub, @Sn, @DS)", cnn.con);
